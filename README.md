...
- [NinjiaTag](#ninjiatag)
  ...
  - [服务端安装部署](#服务端安装部署)
    ...
    - [4.放置服务端key](#4放置服务端key)
    - [5.安装python3相关库](#5安装python3相关库)
      - [基础网络和加密组件](#基础网络和加密组件)
    ...
    - [安装pm2 守护定时执行](#安装pm2-守护定时执行)
      ...
      - [PM2 长期运行脚本命令](#pm2-长期运行脚本命令)
        - [启动脚本并命名进程](#启动脚本并命名进程)
        - [长期运行保障措施](#长期运行保障措施)
        - [pm2常用管理命令(部署时可忽略)](#pm2常用管理命令部署时可忽略)
...
1.  在前端页面有配置服务器地址选项，填入部署的[服务器远程地址](#6服务器后端地址远程)/query，注意加上/query后缀，如`http://服务器ip:3000/query`
